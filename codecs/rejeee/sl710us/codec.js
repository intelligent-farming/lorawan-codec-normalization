...
//
// Normalized payload codec for rejeee/sl710us (Rejeee SL710 LoRaWAN water-
// pressure sensor, submersible, 0-3.5 MPa). Original work; wire format from the
// documented Rejeee sensor-data protocol (SL710 User Manual section 4.2.3;
...
// Hydrostatic pressure Pa -> water.pressure (kPa, /1000). A frame without the
...
function decodeUplinkCore(input) {
  ...
  data.water = { pressure: round(pa / 1000, 3) };
  data.pressurePa = pa;
  return { data: data };
}
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "rejeee"; result.data.model = "sl710us"; }
  return result;
}
