...
//
// Normalized payload codec for RadioBridge RBS301-DWS (Door/Window Sensor:
// open/closed contact state).
//
...
//
// Mapping decisions:
//   0x03 DOOR/WINDOW  byte[2] (0=Closed, 1=Open) -> action.contactState
//                                                   ('closed' | 'open')
...
//
// This is a door/window (reed) contact sensor: the DOOR/WINDOW event maps to
// action.contactState (the contact vocabulary key), NOT action.motion (a known
// upstream copy-paste bug for door sensors). Upstream emits the strings
// "Closed"/"Open"; the vocabulary enum is "closed"/"open", so 0 -> "closed"
// and 1 -> "open".
//
...
function decodeUplinkCore(input) {
  ...
  // ===== Door/Window (0x03) — the primary contact event =====
  if (type === 0x03) {
    if (bytes.length < 3) {
      return { errors: ['door/window payload too short'] };
    }
    data.messageType = 'doorWindow';
    // 0 = Closed, 1 = Open (upstream treats any non-zero as Open).
    data.action = { contactState: bytes[2] === 0 ? 'closed' : 'open' };
    return { data: data };
  }
