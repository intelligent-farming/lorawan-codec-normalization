...
//
// Normalized payload codec for radio-bridge/rbs306-us10m (RBS306-US10M Ultrasonic Level Sensor).
//
...
// the vocabulary: ultrasonic distance (mm) -> tank.distance (m, /1000). Reset/supervisory/undefined frames -> error.
...
function decodeUplinkCore(input) {
  ...
  var data = {};
  if (d.Ultrasonic && typeof d.Ultrasonic.Distance === "number") {
    data.tank = { distance: round(d.Ultrasonic.Distance / 1000, 3) };
    data.distanceMm = d.Ultrasonic.Distance;
    if (d.Ultrasonic.Event !== undefined) { data.event = d.Ultrasonic.Event; }
  }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "radio-bridge"; result.data.model = "rbs306-us10m"; }
  return result;
}
