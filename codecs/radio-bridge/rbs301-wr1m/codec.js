...
//
// Normalized payload codec for RadioBridge RBS301-WR1M (Indoor Water Detect
// Sensor, rope probe): reports a boolean water-leak state.
//
...
//
// Mapping decisions:
//   0x08 WATER        byte[2] (0=Water Present, else Not Present) -> water.leak
//                     (boolean: true = leak detected). byte[3] = relative water
//                     level -> waterRelative camelCase extra.
...
//
// This is a water-leak (rope) sensor: the WATER event maps to water.leak (the
// water-leak vocabulary key). Upstream emits Water.Event "Water Present" /
// "Water Not Present"; we normalize the leak condition to a boolean — byte[2]
// === 0 ("Water Present") -> water.leak true, otherwise false.
//
...
function decodeUplinkCore(input) {
  ...
  // ===== Water (0x08) — the primary leak event =====
  if (type === 0x08) {
    if (bytes.length < 4) {
      return { errors: ['water payload too short'] };
    }
    data.messageType = 'water';
    // Upstream: byte[2] === 0 -> "Water Present", else "Water Not Present".
    data.water = { leak: bytes[2] === 0 };
    // byte[3] = relative water measurement (0-255); vendor diagnostic extra.
