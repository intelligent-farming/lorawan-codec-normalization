...
//
// Normalized payload codec for radio-bridge/rbs301-temp-int (RBS301-TEMP-INT Probeless Temperature Sensor).
//
...
// vocabulary: onboard (CMOS) temperature -> temperature (C). Reset/supervisory/undefined frames -> error.
...
function decodeUplinkCore(input) {
  ...
  var data = {};
  if (d.TempInt && typeof d.TempInt.currentTemperature === "number") { data.temperature = d.TempInt.currentTemperature; }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "radio-bridge"; result.data.model = "rbs301-temp-int"; }
  return result;
}
