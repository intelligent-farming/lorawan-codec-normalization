...
//
// Normalized payload codec for radio-bridge/rbs306-tilt-hp (high-precision tilt sensor).
//
// RadioBridge event decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/radio-bridge/*.js, attributed in
// NOTICE), renamed radioBridgeDecode. decodeUplinkCore maps the event payload to
// the vocabulary (door/contact -> action.contactState, tilt angle -> tilt.angle,
// voltmeter -> power.voltage); reset/supervisory/undefined frames -> error.
...
function radioBridgeDecode(input) {
...
function rbExtras(data, obj, skip) { var k; for (k in obj) { if (k === skip || !Object.prototype.hasOwnProperty.call(obj, k)) { continue; } data[k.charAt(0).toLowerCase() + k.slice(1)] = obj[k]; } }
function decodeUplinkCore(input) {
  var raw = radioBridgeDecode(input);
  var d = (raw && raw.data) || {};
  if (d.Event === "UNDEFINED") { return { errors: ["unsupported or undefined radio-bridge event"] }; }
  var data = {};
  if (d.DoorWindow) { data.action = { contactState: d.DoorWindow.Event === "Open" ? "open" : "closed" }; rbExtras(data, d.DoorWindow, "Event"); }
  else if (d.Contact) { data.action = { contactState: /Open/i.test(d.Contact.Event) ? "open" : "closed" }; rbExtras(data, d.Contact, "Event"); }
  else if (d.Tilt && typeof d.Tilt.Angle === "number") { data.tilt = { angle: d.Tilt.Angle }; rbExtras(data, d.Tilt, "Angle"); }
  else if (d.TiltHP && typeof d.TiltHP.Angle === "number") { data.tilt = { angle: d.TiltHP.Angle }; rbExtras(data, d.TiltHP, "Angle"); }
  else if (d.Voltmeter && typeof d.Voltmeter.Voltage === "number") { data.power = { voltage: d.Voltmeter.Voltage }; rbExtras(data, d.Voltmeter, "Voltage"); }
  else { return { errors: ["frame carries no normalized measurement (" + (d.Type || "unknown") + ")"] }; }
  if (d.Protocol !== undefined) { data.protocol = d.Protocol; }
  if (d.Counter !== undefined) { data.counter = d.Counter; }
  if (d.Type !== undefined) { data.eventType = d.Type; }
  return { data: data };
}
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    ...
    result.data.model = "rbs306-tilt-hp";
  }
  return result;
}
