...
//
// Normalized payload codec for radio-bridge/rbs306-temp-ext (RBS306-TEMP-EXT Outdoor External Temperature Probe).
//
...
// the vocabulary: external probe temperature -> temperature (C). Reset/supervisory/undefined frames -> error.
...
function decodeUplinkCore(input) {
  ...
  var data = {};
  if (d.TempExt && typeof d.TempExt.currentTemperature === "number") {
    data.temperature = d.TempExt.currentTemperature;
    if (typeof d.TempExt.rawMeasurement === "number") { data.rawTemperature = d.TempExt.rawMeasurement; }
  }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "radio-bridge"; result.data.model = "rbs306-temp-ext"; }
  return result;
}
