...
//
// Normalized payload codec for radio-bridge/rbs301-temp-ext (RBS301-TEMP-EXT Indoor External Temperature Probe).
//
...
// vocabulary: external probe temperature -> temperature (C). Reset/supervisory/undefined frames -> error.
...
function decodeUplinkCore(input) {
  ...
  var data = {};
  if (d.TempExt && typeof d.TempExt.currentTemperature === "number") { data.temperature = d.TempExt.currentTemperature; if (typeof d.TempExt.rawMeasurement === "number") { data.rawTemperature = d.TempExt.rawMeasurement; } }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "radio-bridge"; result.data.model = "rbs301-temp-ext"; }
  return result;
}
