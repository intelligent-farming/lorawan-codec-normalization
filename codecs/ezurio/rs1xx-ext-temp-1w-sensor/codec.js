...
//
// Normalized payload codec for Ezurio / Laird Sentrius RS1xx External
// Temperature 1-Wire sensor (with onboard humidity).
//
// Ported/normalized from the upstream Apache-2.0 decoder ("Laird Protocol TTI
// Payload Library", Greg Leach @ Ezurio Connectivity;
// TheThingsNetwork/lorawan-devices vendor/ezurio/rs1xx-ext-temp-1w-sensor.js,
...
// upstream normalizeUplink. The upstream decodeUplink dispatch routes only the
// Temp/RH (0x01), aggregated Temp/RH (0x02), backlog (0x03/0x04), config
// (0x05/0x06), firmware (0x07) and battery-voltage (0x0A) message types; we
// surface the calibrated air.temperature + air.relativeHumidity pair (climate)
// plus battery voltage and device status.
//
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    ...
    result.data.model = "rs1xx-ext-temp-1w-sensor";
  }
  return result;
}
