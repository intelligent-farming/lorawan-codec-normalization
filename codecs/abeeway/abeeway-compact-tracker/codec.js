...
//
// Normalized payload codec for the Abeeway Compact Tracker (multi-mode
// geolocation asset tracker: GPS/GNSS + WiFi/BLE scanning + accelerometer +
// temperature, on the Abeeway "Asset Tracker 2.0" uplink format, fPort 18).
//
...
// vendor/abeeway/asset-tracker-2.js, attributed in NOTICE). The upstream field
// extraction (byte slicing, `decodeCondensed` companding, the 3-byte + "00"
// shift lat/lon assembly) is reproduced faithfully; only the JSON shape is
// re-authored to the normalized vocabulary (never the upstream
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    ...
    result.data.model = "abeeway-compact-tracker";
  }
  return result;
}
