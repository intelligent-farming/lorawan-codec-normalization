...
//
// Normalized payload codec for the RAKwireless / Qingping Temperature & Humidity
// Monitor for LoRaWAN (Qingping Temp & RH Monitor Pro S).
//
// Original work for @intelligent-farming/lorawan-codec-normalization. Wire format
// (the Qingping real-time-data frame) ported from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/rakwireless/decoder-qingping.js, the
// shared Qingping decoder; attributed in NOTICE). We faithfully decode the same
// frame layout and field math as upstream, then NORMALIZE into the shared
// vocabulary instead of emitting upstream's flat keys.
//
...
//   co2         : 2 bytes, big-endian (ppm; the decoder is shared with the CO2
//                 variant, so the field is present on the wire even though the
//                 T&H model has no CO2 sensor and reports 0)
//   battery     : 1 byte
//
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    ...
    result.data.model = "qingping-t-h";
  }
  return result;
}
