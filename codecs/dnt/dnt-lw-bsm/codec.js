...
//
// Normalized payload codec for dnt/dnt-lw-bsm (switch actuator with power metering).
//
// Wire-format decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/dnt/dnt-lw-bsm.js, attributed in
// NOTICE), renamed upstreamDecode (downlink funcs neutralized). decodeUplinkCore maps the
...
function upstreamDecodeDownlink(input) {
  return {
    data: {
      bytes: input.bytes
    },
...
function decodeUplinkCore(input){
  ...
  var data={};
  var k;
  for(k in d){ if(!Object.prototype.hasOwnProperty.call(d,k))continue; var val=d[k]; if(val===null||val===undefined)continue; if(k==="model"){data.deviceModel=val;continue;} if(k==="make"){data.deviceMake=val;continue;} if(k==="active_power"&&val&&val.value!==undefined){var ap=parseFloat(val.value);if(!isNaN(ap)){data.power=data.power||{};data.power.active=ap;}continue;}
