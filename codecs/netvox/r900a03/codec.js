...
//
// Normalized payload codec for Netvox R900A03 (Wireless Water Leak Detector),
// data report on fPort 22.
//
// Original work for @intelligent-farming/lorawan-codec-normalization. Wire
// format understood with reference to the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/netvox/payload/r900a03.js, attributed
...
// the 16-bit big-endian device type (0x010B == 267 == R900A03) and bytes[3] the
...
// bytes[5] is the water-leak state (0x00 == no leak, non-zero == leak) ->
// water.leak (boolean), and bytes[6] is the shock/tamper alarm state (0x00 == no
// alarm, non-zero == alarm), surfaced as the camelCase extra `shockTamperAlarm`.
// This device reports a single leak state and no temperature. Config responses
...
function decodeUplinkCore(input) {
  ...
  // Byte 5: water-leak state. 0x00 == no leak, non-zero == leak.
  data.water = {
    leak: bytes[5] !== 0x00
  };

  // Byte 6: shock/tamper alarm state. Categorical, surfaced as a camelCase extra.
