...
//
// Normalized payload codec for Netvox R31503 (R315 - Wireless Temperature /
// Humidity / PIR / Emergency Button / Tilt / Water Leak Sensor), data report on
// fPort 6.
//
// Ported/normalized from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/netvox/payload/r315.js, attributed in
// NOTICE). The upstream wire format is the source of truth; the normalization to
// the shared vocabulary is authored here (do NOT copy upstream normalizeUplink).
//
// fPort 6 carries periodic data reports; byte0=version, byte1=deviceType (0xD2),
// byte2=reportType discriminator. Battery is volts (bytes[3], high bit is the
// low-battery flag, surfaced as the camelCase extra `lowBattery`). Report 0x01
// carries temperature (air.temperature, 0.01 C two's-complement) and humidity
// (air.relativeHumidity, 0.5 %), gated by bytes[5] bit0. Report 0x02 carries
// illuminance (air.lightIntensity, lux) plus threshold-alarm flags. Report 0x12
// carries temperature, humidity (0.01 %), illuminance and alarm flags.
// Threshold-alarm flags are categorical, surfaced as camelCase extras.
// Device-info (byte2 == 0x00), the sensor-enable/state snapshot (byte2 == 0x11)
// and config responses (fPort 7) carry no measurement and are reported as errors.
