...
//
// Normalized payload codec for netvox/r72632a01 (Netvox R726-series soil NPK
// probe). The R726 firmware is a shared multi-sensor decoder; the r72632a01
// variant reports soil nitrogen/phosphorus/potassium.
//
// Payload decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/netvox/r72632a01.js, attributed in
// NOTICE), renamed netvoxDecode; downlink encoder/decoder renamed to inert
// helpers. decodeUplinkCore maps Nitrogen/Phosphorus/Potassium -> soil.n/p/k,
// Volt -> battery; other shared-codec channels (only present on sibling R726
// variants) pass through as camelCase extras; device-info/config -> error.
...
function decodeUplinkCore(input) {
  ...
  for (k in d) {
    ...
    var val = d[k];
    if (val === null || val === undefined || val === "NoSensor") { continue; }
    if (k === "Nitrogen" && typeof val === "number") { data.soil = data.soil || {}; data.soil.n = val; continue; }
    if (k === "Phosphorus" && typeof val === "number") { data.soil = data.soil || {}; data.soil.p = val; continue; }
    if (k === "Potassium" && typeof val === "number") { data.soil = data.soil || {}; data.soil.k = val; continue; }
    if (k === "Volt" && typeof val === "number") { data.battery = val; continue; }
