...
//
// Normalized payload codec for Netvox R31502 (R315 - Wireless Temperature /
// Humidity / Light / Tilt / Reed Switch / External Vibration Sensor), data
// report on fPort 6.
//
...
// normalizeUplink.
//
// fPort 6 carries periodic data reports; bytes[2] is the report-type
// discriminator. Battery is volts (high bit of bytes[3] is the low-battery
// flag, surfaced as the camelCase extra `lowBattery`). Report 0x01 carries
// temperature (air.temperature, 0.01 C two's-complement) and humidity
// (air.relativeHumidity, 0.5 %); report 0x02 carries illuminance
// (air.lightIntensity, lux); report 0x12 carries temperature, humidity (0.01 %)
// and illuminance. Threshold-alarm flags are categorical, surfaced as camelCase
// extras. Device-info (bytes[2] == 0x00), the sensor-enable/state snapshot
// (bytes[2] == 0x11) and config responses (fPort 7) carry no measurement and
