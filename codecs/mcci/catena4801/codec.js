...
//
// Normalized payload codec for mcci/catena4801 (environmental sensor (temp/humidity/pressure/light)).
//
// Wire-format decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/mcci/catena4801.js, attributed in
// NOTICE). decodeUplinkCore maps tempC->air.temperature, rh->air.relativeHumidity, p->air.pressure, lux->air.lightIntensity, vBat->battery; other fields -> camelCase extras.
...
function upstreamDecoder(bytes, port) {
...
function decodeUplinkCore(input){
  var d=upstreamDecoder(input.bytes,input.fPort)||{};
  if(d.error&&typeof d.error==="string"&&!Object.keys(d).some(function(k){return k!=="error"&&k!=="raw"&&k!=="port";})){return {errors:[d.error]};}
  var data={};
  var k;
  for(k in d){ if(!Object.prototype.hasOwnProperty.call(d,k))continue; var val=d[k]; if(val===null||val===undefined||val==="")continue; if(k==="tempC"&&typeof val==="number"){data.air=data.air||{};data.air.temperature=val;continue;} if(k==="rh"&&typeof val==="number"){data.air=data.air||{};data.air.relativeHumidity=val;continue;} if(k==="p"&&typeof val==="number"){data.air=data.air||{};data.air.pressure=val;continue;} if(k==="lux"&&typeof val==="number"){data.air=data.air||{};data.air.lightIntensity=val;continue;} if(k==="vBat"){if(typeof val==="number")data.battery=val;continue;}
