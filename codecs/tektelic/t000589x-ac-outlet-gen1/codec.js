...
//
// Normalized payload codec for tektelic/t000589x-ac-outlet-gen1 (AC power outlet/switch with energy metering).
//
// Tektelic TLV decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/tektelic/t000589x-ac-outlet-gen1.js, attributed in
// NOTICE), renamed tektelicDecode. decodeUplinkCore maps measurement fields to
// the vocabulary and returns an error for frames the decoder cannot parse.
...
function decodeUplinkCore(input) {
  ...
  for (k in d) {
    ...
    if (k === "rms_voltage") { data.power = data.power || {}; data.power.voltage = val; continue; }
    if (k === "rms_current") { data.power = data.power || {}; data.power.current = val; continue; }
    if (k === "active_power") { data.power = data.power || {}; data.power.active = val; continue; }
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    ...
    result.data.model = "t000589x-ac-outlet-gen1";
  }
  return result;
}
