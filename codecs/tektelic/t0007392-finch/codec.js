...
//
// Normalized payload codec for tektelic/t0007392-finch (Tektelic environmental sensor).
//
// Tektelic TLV decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/tektelic/t0007392-finch.js, attributed in
// NOTICE), renamed tektelicDecode. decodeUplinkCore maps temperature/humidity
// -> air.temperature/air.relativeHumidity; other channels -> camelCase extras;
// unparseable frames -> error.

function tektelicDecode(input) {
    var sensor = 
    {
      "10": {
        "0x00 0xBA": [
...
function decodeUplinkCore(input) {
  var raw = tektelicDecode(input);
  var d = (raw && raw.data) || raw || {};
  if (d.error) { return { errors: [String(d.error)] }; }
  var data = {};
  var k;
  for (k in d) {
    if (!Object.prototype.hasOwnProperty.call(d, k)) { continue; }
    if (k === 'raw' || k === 'port') { continue; }
    var val = d[k];
    if (val === null || val === undefined) { continue; }
    if (k === 'temperature' || k === 'ambient_temperature') { data.air = data.air || {}; data.air.temperature = val; continue; }
    if (k === 'relative_humidity') { data.air = data.air || {}; data.air.relativeHumidity = val; continue; }
    if (k === 'battery_voltage') { data.battery = val; continue; }
...
function decodeUplink(input){var result=decodeUplinkCore(input);if(result&&result.data){result.data.make="tektelic";result.data.model="t0007392-finch";}return result;}
