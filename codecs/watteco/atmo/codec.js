...
//
// Normalized payload codec for Watteco Atm'O (indoor atmospheric sensor:
// temperature, relative humidity, barometric pressure, CO2).
//
...
// vendor/watteco/atmo.js, attributed in NOTICE). Ported from the upstream
...
// (bytes 2-3), a 16-bit attribute id (bytes 4-5), a report-parameters byte
// (byte 6), and then the attribute value from byte 7 onward. This codec
// decodes only the measurement-bearing clusters of the Atm'O and maps them to
...
//   cluster 0x0403 (1027) pressure    -> air.pressure         (signed16 hPa, atmospheric)
//   cluster 0x800C (32780) CO2        -> air.co2              (unsigned16 ppm)
...
function decodeUplinkCore(input) {
  ...
  if (cmd === 0x0a) {
    ...
    if (cluster === 1027 && attr === 0) {
      // Barometric pressure: signed 16-bit hPa (no scaling). The Atm'O reports
    ...
    if (cluster === 32780 && attr === 0) {
      // CO2 concentration: unsigned 16-bit ppm.
      data.air = { co2: u16be(bytes[h], bytes[h + 1]) };
      return { data: data };
    }
