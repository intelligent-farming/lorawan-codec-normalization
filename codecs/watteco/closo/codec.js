...
//
// Normalized payload codec for Watteco Clos-O (dry-contact opening sensor): binary input -> action.contactState.
//
...
function decodeUplinkCore(input) {
  ...
  if (cmd === 0x0a) {
    ...
    if (cluster === 15 && attr === 85) {
      // Binary input present value: contact state (1 = open, 0 = closed).
      data.action = { contactState: (bytes[h] & 0x01) ? 'open' : 'closed' };
      return { data: data };
    }
    if (cluster === 15 && attr === 1026) {
      // Binary-input change counter.
      data.index = u32be(bytes[h], bytes[h + 1], bytes[h + 2], bytes[h + 3]);
      return { data: data };
    }
