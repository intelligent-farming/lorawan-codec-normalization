...
//
// Normalized payload codec for Watteco Inclino (vertical tilt / inclination
// angle sensor).
//
...
function decodeUplinkCore(input) {
  ...
  if (cmd === 0x0a) {
    ...
    if (cluster === 12 && attr === 85) {
      // Inclination angle: IEEE754 float32, big-endian, in degrees.
      var bits = u32be(bytes[h], bytes[h + 1], bytes[h + 2], bytes[h + 3]);
      var angle = f32(bits);
      if (!isFinite(angle)) {
        return { errors: ['angle attribute decoded to a non-finite value'] };
      }
      data.tilt = { angle: round(angle, 3) };
      return { data: data };
    }
