...
//
// Normalized payload codec for fludia/fm432e-10-15mn (Fludia FM432e electronic
// electricity-meter reader, 10/15-minute reporting).
//
// Wire-format decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/fludia/fm432e-10-15mn-decode.js,
// attributed in NOTICE), renamed fm432eDecode. decodeUplinkCore maps the
// cumulative index (Wh) -> metering.energy.total and the most-recent derived
// per-interval power (W) -> power.active; the power/increment series travel as
// camelCase extras. Unrecognized frames -> error.
...
function decodeUplinkCore(input){
  var raw=fm432eDecode(input); var d=(raw&&raw.data)||{};
  if(raw&&raw.errors&&raw.errors.length){return {errors:raw.errors};}
  if(d.message_type===null||d.message_type===undefined){return {errors:["unrecognized frame"]};}
  var data={};
  if(d.index!==undefined&&d.index!==null&&d.index>=0){data.metering={energy:{total:d.index}};}
  if(d.powers&&d.powers.length){var latest=null;for(var i=d.powers.length-1;i>=0;i--){if(d.powers[i]!==null&&d.powers[i]!==undefined){latest=round(d.powers[i],3);break;}}if(latest!==null){data.power={active:latest};}data.powers=d.powers.map(function(x){return round(x,3);});if(d.increments)data.increments=d.increments;}
  data.messageType=d.message_type;
  if(d.time_step!==undefined&&d.time_step!==null)data.timeStep=d.time_step;
  return {data:data};
}

// Device identity (make/model), emitted on every successful decode. See AUTHORING.md.
function decodeUplink(input){var result=decodeUplinkCore(input);if(result&&result.data){result.data.make="fludia";result.data.model="fm432e-10-15mn";}return result;}
