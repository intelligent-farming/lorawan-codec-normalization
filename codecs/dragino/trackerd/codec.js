...
//
// Normalized payload codec for dragino/trackerd (GPS tracker with temperature/humidity).
//
// Wire-format decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/dragino/trackerd.js, attributed in
// NOTICE). decodeUplinkCore maps Latitude/Longitude->position, Tem->air.temperature, Hum->air.relativeHumidity, BatV->battery; other fields -> camelCase extras.
...
function decodeUplinkCore(input){
  ...
  var data={};
  var k;
  for(k in d){ if(!Object.prototype.hasOwnProperty.call(d,k))continue; var val=d[k]; if(val===null||val===undefined||val==="")continue; if(k==="Latitude"&&typeof val==="number"){data.position=data.position||{};data.position.latitude=val;continue;} if(k==="Longitude"&&typeof val==="number"){data.position=data.position||{};data.position.longitude=val;continue;} if(k==="Tem"&&typeof val==="number"){data.air=data.air||{};data.air.temperature=val;continue;} if(k==="Hum"&&typeof val==="number"){data.air=data.air||{};data.air.relativeHumidity=val;continue;} if(k==="BatV"||k==="Bat"){if(typeof val==="number")data.battery=val;continue;}
