...
//
// Normalized payload codec for dragino/lsn50-v2 (multi-probe sensor (SHT temp/humidity + door contact)).
//
// Wire-format decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/dragino/lsn50-v2.js, attributed in
// NOTICE). decodeUplinkCore maps TempC_SHT->air.temperature, Hum_SHT->air.relativeHumidity, Door_status->action.contactState, BatV->battery; other fields -> camelCase extras.
...
function decodeUplinkCore(input){
  ...
  var data={};
  var k;
  for(k in d){ if(!Object.prototype.hasOwnProperty.call(d,k))continue; var val=d[k]; if(val===null||val===undefined||val==="")continue; if(k==="TempC_SHT"&&typeof val==="number"){data.air=data.air||{};data.air.temperature=val;continue;} if(k==="Hum_SHT"&&typeof val==="number"){data.air=data.air||{};data.air.relativeHumidity=val;continue;} if(k==="Door_status"){data.action=data.action||{};data.action.contactState=(/open/i.test(val))?"open":"closed";continue;} if(k==="BatV"||k==="Bat"){if(typeof val==="number")data.battery=val;continue;}
