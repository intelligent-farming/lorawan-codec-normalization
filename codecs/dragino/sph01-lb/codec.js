...
//
// Normalized payload codec for dragino/sph01-lb (Dragino SPH01-LB/LS soil pH +
// soil temperature sensor). Authored from the upstream Apache-2.0 Dragino
// decoder (attributed in NOTICE; upstream stores JS with escaped newlines).
//
// fPort 2: battery ((b0<<8|b1)&0x3FFF)/1000; DS18B20 probe temp b2..3 signed/10
// (extra); soil pH b4..5 /100 -> soil.pH; soil temperature b6..7 signed/10 ->
// soil.temperature; interrupt flag b8, message type b10 -> extras.
...
function decodeUplinkCore(input) {
  ...
  data.soil = { pH: round((((b[4] & 0xff) << 8) | b[5]) / 100, 2), temperature: round(s16(b[6], b[7]) / 10, 2) };
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "dragino"; result.data.model = "sph01-lb"; }
  return result;
}
