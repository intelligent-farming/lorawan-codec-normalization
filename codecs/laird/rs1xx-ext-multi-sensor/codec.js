...
//
// Normalized payload codec for Laird (Ezurio) Sentrius RS1xx External
// Multi-Sensor.
//
// Ported/normalized from the upstream Apache-2.0 decoder ("Laird Protocol TTI
// Payload Library", Greg Leach @ Laird/Ezurio Connectivity;
// TheThingsNetwork/lorawan-devices vendor/laird/rs1xx-ext-multi-sensor.js,
...
// Laird = Ezurio's former brand: this is the same wire protocol as the
// ezurio/rs1xx-temp-rh-sensor codec, with the open/closed (contact) sensor
// frames the multi-sensor variant additionally emits.
//
...
// voltage on the dedicated 0x0A frame (vocabulary `battery`, in volts). The
// open/closed sensor state (0x09) maps to the vocabulary action.contactState
// ("open" / "closed").
...
function decodeTime(b0, b1, b2, b3) {
  var secs = (b0 * 16777216 + (b1 << 16) + (b2 << 8) + b3) + 1420070400;
...
function decodeUplinkCore(input) {
  ...
  // 0x09 - Open/closed (contact) sensor state (6 bytes): options, operating
  // mode, state, alert cancellation, counter. State maps to the vocabulary
  // action.contactState.
  if (msgType === 0x09) {
    if (bytes.length !== 6) {
      return { errors: ['Invalid uplink message length!'] };
    ...
    var data9 = {
      ...
      action: {
        contactState: bytes[3] === 1 ? 'open' : 'closed'
      },
      operatingMode: OPERATING_MODE[bytes[2]],
      alertCancellation: bytes[4] === 1,
      contactCounter: bytes[5],
      options: decodeOptions(bytes[1])
    };
    return { data: data9 };
  }
