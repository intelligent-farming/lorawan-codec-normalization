...
//
// Normalized payload codec for strega/motorized-valve (Strega motorized valve;
// its periodic uplink carries a box temperature and humidity).
//
// Uplink wire-format decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/strega/motorized-valve*.js, attributed
// in NOTICE), renamed stregaDecode; the upstream downlink encoder/decoder are
// renamed to inert helpers. decodeUplinkCore normalizes: box temperature/humidity
// -> air.temperature / air.relativeHumidity (climate); battery % -> batteryPercent;
// valve/leak/limit-switch/DI status as camelCase extras.
...
function stregaEncodeDownlink(input) {
 ...
 function stregaDecodeDownlink(input) {
   return {
     data: {
       bytes: input.bytes
     },};}
...
function decodeUplinkCore(input) {
  ...
  for (k in d) {
    ...
    if (k === "Temp") { if (typeof val === "number") { data.air = data.air || {}; data.air.temperature = val; } continue; }
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    result.data.make = "strega";
    result.data.model = "motorized-valve";
  }
  return result;
}
