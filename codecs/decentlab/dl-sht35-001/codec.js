...
//
// Normalized payload codec for Decentlab DL-SHT35 (SHT35 air temperature and
// relative humidity sensor with radiation shield for LoRaWAN).
//
...
// 16-bit big-endian words) ported/normalized from the upstream Apache-2.0
// decoder (TheThingsNetwork/lorawan-devices vendor/decentlab/dl-sht35.js,
// attributed in NOTICE). The upstream decodeUplink emits raw Decentlab
// per-sensor objects; the per-sensor conversion formulas below are ported
// faithfully and the results are then mapped onto the shared normalized
// vocabulary.
//
// Mapping: air_temperature -> air.temperature (°C); air_humidity ->
// air.relativeHumidity (%); battery_voltage (already volts) -> battery.
...
function decodeUplinkCore(input) {
  ...
  //   bit0 temperature/humidity (2 words), bit1 battery (1 word)
  var lengths = [2, 1];
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    ...
    result.data.model = "dl-sht35-001";
  }
  return result;
}
