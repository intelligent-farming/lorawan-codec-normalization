...
//
// Normalized payload codec for Decentlab DL-PR36 (high-precision pressure /
// liquid-level and temperature sensor for boreholes, wells and piezometers on
// LoRaWAN). Despite reporting a "pressure", this is a water-column probe (vendor
// product page: "high-precision pressure / liquid level and temperature
// sensor"), not an atmospheric barometer: the upstream pressure transfer
// function spans a configurable hydrostatic range scaled by kp, so the reading
// is hydrostatic liquid pressure, mapped to water.pressure (groundwater
// category) — never to air.pressure (atmospheric, 900-1100 hPa).
//
// Ported/normalized from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/decentlab/dl-pr36-8192.js, attributed
...
//   - Probe temperature: upstream (x - 32768) / 256 gives degC, maps to
//     water.temperature.current.
//   - Battery voltage: already volts (x / 1000), maps directly to `battery`.
//   - Protocol version and device id are device metadata the vocabulary does
//     not model and are emitted as the camelCase extras `protocolVersion` and
//     `deviceId`.

// device-specific calibration parameter (kp from the -8192 variant)
var KP = 8192;
...
function decodeUplinkCore(input) {
  ...
  var data = { protocolVersion: version, deviceId: deviceId };
  var water = {};
  var hasWater = false;
  ...
  if (words[0]) {
    ...
    water.temperature = { current: round((w[1] - 32768) / 256, 4) };
