...
//
// Normalized payload codec for Decentlab DL-SHT35-002 (Air Temperature and
...
// (TheThingsNetwork/lorawan-devices vendor/decentlab/dl-sht35.js, attributed in
...
// air.relativeHumidity; battery_voltage (already volts) -> battery. The SHT35
// is an air sensor only (no external/water probe on this variant), so there is
// no water.temperature mapping. The device's protocol version and device id are
// not measurement data and are dropped.
...
function decodeUplinkCore(input) {
  ...
  //   bit0 air temperature + humidity (2 words), bit1 battery voltage (1 word)
  var lengths = [2, 1];
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    ...
    result.data.model = "dl-sht35-002";
  }
  return result;
}
