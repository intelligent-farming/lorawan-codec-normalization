...
//
// Normalized payload codec for mcf88/mcf-lw12plg (MCF88 LW12 electricity meter).
//
// Wire-format decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/mcf88/mcf-lw12plg.js, attributed in
// NOTICE), renamed mcfDecode. The metering frame (uplink id 0x09) carries
// active/reactive/apparent energy (Wh) and, in its long form, active power (W),
// RMS voltage (V) and current (mA). decodeUplinkCore maps activeEnergy ->
// metering.energy.total, activePower -> power.active, voltage -> power.voltage,
// current (mA) -> power.current (A); other fields (reactive/apparent, period,
// frequency, date) travel as extras.

"use strict";

function mcfDecode(payload) {
...
function decodeUplinkCore(input){
  var raw=mcfDecode(input); var d=(raw&&raw.data)||raw||{};
  if(!d||typeof d!=="object"){return {errors:["unrecognized frame"]};}
  var data={};
  var k;
  for(k in d){ if(!Object.prototype.hasOwnProperty.call(d,k))continue; var val=d[k]; if(val===null||val===undefined)continue;
    if(k==="activeEnergy"){var ae=parseInt(val,10);if(!isNaN(ae)){data.metering={energy:{total:ae}};}continue;}
    if(k==="activePower"){var ap=parseInt(val,10);if(!isNaN(ap)){data.power=data.power||{};data.power.active=ap;}continue;}
    if(k==="voltage"){var v=parseFloat(val);if(!isNaN(v)){data.power=data.power||{};data.power.voltage=v;}continue;}
    if(k==="current"){var cu=parseInt(val,10);if(!isNaN(cu)){data.power=data.power||{};data.power.current=round(cu/1000,5);}continue;}
    data[k]=val;
  }
  return {data:data};
}
...
function decodeUplink(input){var result=decodeUplinkCore(input);if(result&&result.data){result.data.make="mcf88";result.data.model="mcf-lw12plg";}return result;}
