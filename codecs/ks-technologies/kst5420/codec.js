...
//
// Normalized payload codec for ks-technologies/kst5420 (KS Technologies GPS tracker).
//
// Wire-format decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/ks-technologies/kst5420.js, attributed
// in NOTICE), renamed ksDecode. decodeUplinkCore maps latitude/longitude ->
// position.latitude/longitude, battery -> battery; altitude/accuracy/speed and
// other fields -> camelCase extras.

function ksDecode(input) {
...
function decodeUplinkCore(input){
  var raw=ksDecode(input); var d=(raw&&raw.data)||raw||{};
  var data={};
  var k;
  for(k in d){ if(!Object.prototype.hasOwnProperty.call(d,k))continue; var val=d[k]; if(val===null||val===undefined)continue;
    if(k==="latitude"&&typeof val==="number"){data.position=data.position||{};data.position.latitude=val;continue;}
    if(k==="longitude"&&typeof val==="number"){data.position=data.position||{};data.position.longitude=val;continue;}
    if(k==="battery"&&typeof val==="number"){data.battery=val;continue;}
...
function decodeUplink(input){var result=decodeUplinkCore(input);if(result&&result.data){result.data.make="ks-technologies";result.data.model="kst5420";}return result;}
