...
//
// Normalized payload codec for iot-factory/m-bus-gateway - a LoRaWAN M-Bus
// gateway that relays wired M-Bus / wM-Bus meter frames (electricity, water,
// heat, gas). The wire-format decoder below is ported verbatim from the upstream
// Apache-2.0 decoder (TheThingsNetwork/lorawan-devices
// vendor/iot-factory/m-bus-gateway.js, attributed in NOTICE), renamed to
// mbusDecode; decodeUplinkCore adds the normalization (authored here, NOT copied
// from any upstream normalizeUplink).
//
// Mapping: the most recent valid Electricity energy register (desc=energy,
// unit=Wh) -> metering.energy.total (Wh). Gateway diagnostics (battery %, power
// type, serial, protocol version) and the energy frame meter serial / medium /
// time travel as camelCase extras.
...
function decodeUplinkCore(input) {
  var raw = mbusDecode(input);
  if (raw && raw.errors && raw.errors.length) { return { errors: raw.errors }; }
  var d = (raw && raw.data) || {};
  var data = {};
  if (typeof d.battery_pct === "number") { data.batteryPercent = d.battery_pct; }
  if (d.type_power !== undefined && d.type_power !== null) { data.powerType = d.type_power; }
  if (d.sn !== undefined && d.sn !== null) { data.serialNumber = d.sn; }
  if (d.version_protocol !== undefined && d.version_protocol !== null) { data.protocolVersion = d.version_protocol; }
  var frames = d.frames || [];
  var energyWh = null, meterSn, frameTime, medium;
  var i;
  for (i = 0; i < frames.length; i++) {
    var f = frames[i];
    var m = f && f.measurement;
    if (m && m.desc === "energy" && m.unit === "Wh" && typeof m.value === "number") {
      energyWh = m.value; meterSn = f.meter_serial_number; frameTime = f.frame_isotime; medium = f.medium;
    }
  }
  if (energyWh !== null) {
    data.metering = { energy: { total: energyWh } };
    if (meterSn !== undefined) { data.meterSerialNumber = meterSn; }
    if (frameTime !== undefined) { data.frameTime = frameTime; }
    if (medium !== undefined) { data.medium = medium; }
  }
  var out = { data: data };
  if (raw && raw.warnings && raw.warnings.length) { out.warnings = raw.warnings; }
  return out;
}
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    ...
    result.data.model = "m-bus-gateway";
  }
  return result;
}
