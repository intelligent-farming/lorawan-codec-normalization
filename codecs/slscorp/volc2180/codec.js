...
//
// Normalized payload codec for slscorp/volc2180 (SLS streetlight controller with
// electrical metering).
//
// Wire-format decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/slscorp/volc2180.js, attributed in
// NOTICE), renamed slscorpDecode. The meter-data uplink (MIB GetResponse 0x58)
// yields power/energy/voltage(VRMS)/current(IRMS). decodeUplinkCore maps power ->
// power.active (W), energy (kWh) -> metering.energy.total (Wh), vrms ->
// power.voltage (V), irms -> power.current (A); dimmer and other fields -> extras.

function slscorpDecode(input) {
...
function decodeUplinkCore(input){
  var raw=slscorpDecode(input); var d=(raw&&raw.data)||raw||{};
  var data={};
  var k;
  for(k in d){ if(!Object.prototype.hasOwnProperty.call(d,k))continue; var val=d[k]; if(val===null||val===undefined)continue;
    ...
    if(k==="power"&&!isNaN(num)){data.power=data.power||{};data.power.active=num;continue;}
    if(k==="energy"&&!isNaN(num)){data.metering=data.metering||{};data.metering.energy={total:round(num*1000,3)};continue;}
    if(k==="vrms"&&!isNaN(num)){data.power=data.power||{};data.power.voltage=num;continue;}
    if(k==="irms"&&!isNaN(num)){data.power=data.power||{};data.power.current=num;continue;}
    data[k]=val;
  }
  return {data:data};
}
...
function decodeUplink(input){var result=decodeUplinkCore(input);if(result&&result.data){result.data.make="slscorp";result.data.model="volc2180";}return result;}
