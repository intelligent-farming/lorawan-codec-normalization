...
//
// Normalized payload codec for nexelec/origin (Nexelec smoke + indoor air-quality
// alarm). Its Real-Time message (message type 0x04) reports ambient temperature
// and humidity.
//
// Wire-format decoder ported verbatim from the upstream Apache-2.0 decoder
// (TheThingsNetwork/lorawan-devices vendor/nexelec/origin.js, attributed in
// NOTICE), renamed nexelecDecode. decodeUplinkCore maps temperature.value ->
// air.temperature and humidity.value -> air.relativeHumidity; other reported
// objects are flattened to their value as camelCase extras.



function nexelecDecode(input) 
{
    var tab_bin=[];
    var output = 0;
    var data;
...
function decodeUplinkCore(input){
  var raw=nexelecDecode(input); var d=(raw&&raw.data)||raw||{};
  var data={};
  var k;
  for(k in d){ if(!Object.prototype.hasOwnProperty.call(d,k))continue; var val=d[k]; if(val===null||val===undefined)continue;
    if(k==="temperature"&&val&&typeof val==="object"&&typeof val.value==="number"){data.air=data.air||{};data.air.temperature=val.value;continue;}
    if(k==="humidity"&&val&&typeof val==="object"&&typeof val.value==="number"){data.air=data.air||{};data.air.relativeHumidity=val.value;continue;}
    if(val&&typeof val==="object"&&typeof val.value!=="undefined"&&!Array.isArray(val)){data[k]=val.value;continue;}
    data[k]=val;
  }
  return {data:data};
}
...
function decodeUplink(input){var result=decodeUplinkCore(input);if(result&&result.data){result.data.make="nexelec";result.data.model="origin";}return result;}
