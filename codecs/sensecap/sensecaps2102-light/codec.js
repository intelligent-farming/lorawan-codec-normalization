...
//
// Normalized payload codec for SenseCAP S2102 (LoRaWAN Light Intensity Sensor).
//
...
// 4098 = Air Humidity (%RH), 4099 = Light Intensity (lux) — confirmed against the
// sibling SenseCAP S2120 8-in-1 decoder which maps measurementId 4099 to
// "Light Intensity". dataID 7 carries battery percentage + reporting interval.
//
  ...
  for (var i = 0; i + 4 <= hex.length - 4; i += frameHexLen) {
    ...
    if (dataID > 4096) {
      ...
      if (dataID === 4097) {
        ...
        air.lightIntensity = round(value, 0);
        hasAir = true;
        hasTelemetry = true;
      } else {
        // Telemetry the vocabulary does not model -> camelCase extra.
      ...
      var be = leReverseHex(valueHex);
      var u = parseInt(be, 16);
      var batteryPercent = u & 0xffff;
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    ...
    result.data.model = "sensecaps2102-light";
  }
  return result;
}
