...
//
// Normalized payload codec for SenseCAP (SeeedStudio) Wireless Air Temperature
// and Humidity Sensor - LoRaWAN.
//
...
// `messages` array (report_telemetry / upload_battery / upload_sensor_id ...) is
// NOT reproduced.
//
...
// measurement IDs for this device: 4097 air temperature (C), 4098 air humidity
// (%). Battery (special dataID 7) is a percentage -> `batteryPercent` extra
// (the vocabulary `battery` is volts, not percent). Sensor-id / version /
// interval / remove-sensor frames carry no measurement and are ignored.
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    ...
    result.data.model = "sensecap-air-th";
  }
  return result;
}
