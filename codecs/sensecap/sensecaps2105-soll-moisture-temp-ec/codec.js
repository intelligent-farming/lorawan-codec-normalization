...
//
// Normalized payload codec for SenseCAP S2105 (LoRaWAN Soil Moisture,
// Temperature, and EC Sensor). Soil VWC (%), soil temperature (degC), and soil
...
// natural unit. SenseCAP S210x measurement IDs used by the S2105:
//   4102 (0x1006) -> soil temperature (degC)        -> soil.temperature
//   4103 (0x1007) -> soil moisture / VWC (%)         -> soil.moisture
//   4108 (0x100C) -> soil electrical conductivity    -> soil.ec
// The device encodes EC as (dS/m * 1000) on the wire, so upstream's uniform
// "/1000" decode yields dS/m directly (matching the datasheet 0-23 dS/m range
// and the vocabulary soil.ec unit, dS/m); no further conversion is applied.
  ...
  for (var f = 0; f < frameArray.length; f++) {
    ...
    if (checkDataIdIsMeasureUpload(dataID)) {
      ...
      if (dataID === 4102) {
        ...
        // Soil EC. realDataValue is already in dS/m (device encodes dS/m*1000).
        soil.ec = round(realDataValue, 2);
        hasSoil = true;
      } else {
