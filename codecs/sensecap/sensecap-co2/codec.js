...
//
// Normalized payload codec for SenseCAP (SeeedStudio) Wireless CO2 Sensor -
// LoRaWAN.
//
...
//
// Ported from upstream decodeUplink: the CRC16 trailer check, the 7-byte frame
// split, the little-endian dataID, the two's-complement /1000 telemetry value
// and the special dataID-7 battery/interval split are faithful ES5 ports of the
// upstream decoder. The upstream `messages` array (report_telemetry /
// upload_battery / upload_sensor_id ...) is NOT reproduced; the normalization
// below is authored here, mapping each telemetry frame onto the shared
// vocabulary by its SenseCAP measurement ID.
//
...
// measurement IDs for this device:
...
// Battery (special dataID 7) is a PERCENTAGE -> `batteryPercent` extra (the
// vocabulary `battery` is volts, not percent); the reporting interval from the
// same frame becomes the `reportingInterval` extra (seconds, = upstream
// interval * 60). Other telemetry IDs become camelCase extras. Sensor-id /
// version / remove-sensor frames carry no measurement and are ignored.
...
function decodeUplinkCore(input) {
  ...
  for (var f = 0; f < frameCount; f++) {
    ...
    if (dataID > 4096) {
      ...
      // Special battery && interval frame. Low 16 bits = battery percent,
      // high 16 bits = reporting interval (minutes; upstream * 60 -> seconds).
      data.batteryPercent = leUint(bytes, base + 3, 2);
      data.reportingInterval = leUint(bytes, base + 5, 2) * 60;
      measurementCount++;
    }
    // All other dataIDs (version, sensor-id, remove-sensor, unknown) carry no
    // normalized measurement and are intentionally ignored.
  }
