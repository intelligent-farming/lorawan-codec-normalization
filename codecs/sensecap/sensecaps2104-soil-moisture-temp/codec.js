...
//
// Normalized payload codec for SenseCAP S2104 (LoRaWAN Soil Moisture and
// Temperature Sensor).
//
...
// vendor/sensecap/sensecap210x-common-decoder.js a.k.a. the "SenseCAP & TTN
// (new v3) Converter", attributed in NOTICE).
//
...
// natural unit. SenseCAP S210x measurement IDs reported by the S2104:
//   4102 (0x1006) -> soil temperature (degC)           -> soil.temperature
//   4103 (0x1007) -> soil volumetric water content (%) -> soil.moisture
// (The S210x family also defines 4097 air temperature, 4098 air humidity,
// 4099 light, 4100 CO2, 4101 pressure, 4104 soil EC; the S2104 hardware only
// emits the two soil channels. Any other telemetry id seen on the wire is
// surfaced as the camelCase extra `measurement<ID>` rather than silently
// dropped.) Other dataIds are device control/metadata frames (version, sensor
// EUI, battery + interval, remove-sensor). Battery is reported as a PERCENTAGE;
// the vocabulary's `battery` is volts, so it is emitted as the camelCase extra
...
function decodeUplinkCore(input) {
  ...
  for (var f = 0; f < frameArray.length; f++) {
    ...
    if (checkDataIdIsMeasureUpload(dataID)) {
      ...
      if (dataID === 4102) {
        soil.temperature = round(realDataValue, 1);
        hasSoil = true;
      } else if (dataID === 4103) {
        // Soil volumetric water content (%). realDataValue is already in %.
        soil.moisture = round(realDataValue, 1);
        hasSoil = true;
      } else {
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    ...
    result.data.model = "sensecaps2104-soil-moisture-temp";
  }
  return result;
}
