...
//
// Normalized payload codec for ATIM ACW-TCR (Temperature/Humidity Probe).
//
...
// we re-derive only the TCR-relevant frames (measurement + life) directly and
// emit normalized vocabulary keys. We do NOT copy upstream normalizeUplink.
//
// The ACW-TCR is an inertial temperature + precise humidity probe; its
// measurement frame carries a temperature block and a humidity block only.
//
// Mappings:
...
// are dropped and surfaced as warnings, matching upstream's "erreur"
// placeholder without polluting numeric vocabulary fields.
...
function decodeMeasurement(bytes, frame) {
  ...
  // The TCR measurement frame carries no history/sampling header (a single
  ...
  while (i < bytes.length) {
    ...
    // TCR has a single temperature/humidity channel (voie 0), so mask them.
  ...
  var hasAir =
    air.temperature !== undefined || air.relativeHumidity !== undefined;
  if (!hasAir) {
