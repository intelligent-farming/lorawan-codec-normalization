...
//
// Normalized payload codec for ATIM ACW-ILB30 (Outdoor Barrier PIR motion
// sensor / people-counting beam). Category: motion.
//
...
// ports only the "Produit PIR" legacy frame that the ILB30 emits and normalizes
// it to the shared vocabulary. We author the normalization here; we do NOT
// reuse upstream normalizeUplink/postProcess output.
//
  ...
  if (bytes[0] !== 0x32) {
    return {
      errors: ['unsupported ATIM frame type 0x' + bytes[0].toString(16) +
        ' (this codec decodes only the ILB30 PIR frame 0x32)']
    };
  }
  if (bytes.length < 9) {
