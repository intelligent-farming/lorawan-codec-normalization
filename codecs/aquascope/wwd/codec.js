...
//
// Normalized payload codec for Aquascope WWD (Aqua-Scope Flood Sensor).
//
...
//
// Upstream decodes two frames:
//   fPort 10 (status):  byte0 bit0 = leak, byte0 bit1 = remote-probe alarm,
...
//
// Upstream emits `leak` as the raw 0/1 bit; the water-leak vocabulary key is a
// boolean, so it is coerced to true/false. `battery` here is a 0-100 percentage,
// not volts; the vocabulary `battery` is volts, so it is emitted as the camelCase
// extra `batteryPercent`. The remote-probe alarm flag (byte0 bit1) has no
// vocabulary key and is kept as the camelCase extra `remoteAlarm` (0/1, as
// upstream reports it).
...
function decodeUplinkCore(input) {
  ...
  if (input.fPort === 10) {
    ...
    data.remoteAlarm = (bytes[0] & 0x02) ? 1 : 0;
  }
