...
//
// Normalized payload codec for milesight-iot/wt303 (Milesight smart fan-coil
// thermostat reporting ambient temperature and humidity plus setpoint/valve
// state).
//
// Milesight TLV uplink decoder ported verbatim from the upstream Apache-2.0
// decoder (TheThingsNetwork/lorawan-devices vendor/milesight-iot/wt303.js,
// attributed in NOTICE), renamed milesightDecode; decodeUplinkCore normalizes
// temperature/humidity -> air.temperature/air.relativeHumidity (climate),
// battery % -> batteryPercent, and other channels (target temperature, control/
// valve status) to camelCase extras.
...
function decodeUplinkCore(input) {
  ...
  for (k in d) {
    ...
    if (k === "temperature" && typeof val === "number") { data.air = data.air || {}; data.air.temperature = val; continue; }
    if (k === "humidity" && typeof val === "number") { data.air = data.air || {}; data.air.relativeHumidity = val; continue; }
