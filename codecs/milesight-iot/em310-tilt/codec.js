...
//
// Normalized payload codec for milesight-iot/em310-tilt (Milesight 3-axis tilt
// sensor).
//
// Milesight TLV uplink decoder ported verbatim from the upstream Apache-2.0
// decoder (TheThingsNetwork/lorawan-devices vendor/milesight-iot/em310-tilt.js,
// attributed in NOTICE), renamed milesightDecode. decodeUplinkCore maps
// angle_x/y/z (degrees) -> tilt.x/y/z; battery % -> batteryPercent; threshold/
// status fields -> camelCase extras.
...
function decodeUplinkCore(input) {
  ...
  for (k in d) {
    ...
    if (k === "angle_x" && typeof val === "number") { data.tilt = data.tilt || {}; data.tilt.x = val; continue; }
    if (k === "angle_y" && typeof val === "number") { data.tilt = data.tilt || {}; data.tilt.y = val; continue; }
    if (k === "angle_z" && typeof val === "number") { data.tilt = data.tilt || {}; data.tilt.z = val; continue; }
