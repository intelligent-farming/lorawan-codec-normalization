...
//
// Normalized payload codec for Milesight AM319L (Indoor Air Quality Sensor,
// 9-in-1: temperature, humidity, PIR activity, light level, CO2, TVOC,
// barometric pressure, HCHO, PM2.5, PM10).
//
...
// vendor/milesight-iot/am319l.js, attributed in NOTICE). Do NOT copy upstream
...
// 0x05/0x00 PIR trigger/idle (1 byte; trigger -> action.motion.detected true,
// action.motion.count 1; the AM107-style 0x05/0x6A activity-count form is also
// accepted), 0x06/0xCB light level (byte 0-6 ambient-light index, NOT lux ->
// camelCase extra lightLevel; it is not air.lightIntensity, whose unit is lux,
// so this device does not qualify for the `light` category), 0x07/0x7D CO2
// (uint16 ppm -> air.co2), 0x08/0x7D TVOC IAQ (uint16 /100 -> camelCase extra
// tvoc; no vocabulary key), 0x08/0xE6 TVOC (uint16 µg/m3 -> camelCase extra
// tvoc), 0x09/0x73 pressure (uint16 /10 hPa -> air.pressure), 0x0A/0x7D HCHO
// (uint16 /100 mg/m3 -> camelCase extra hcho), 0x0B/0x7D PM2.5 (uint16 µg/m3 ->
// camelCase extra pm2_5), 0x0C/0x7D PM10 (uint16 µg/m3 -> camelCase extra
// pm10), 0x0E/0x01 buzzer/beep (byte -> camelCase extra beep boolean).
...
function decodeUplinkCore(input) {
  ...
  while (i + 1 < bytes.length) {
    ...
    if (channel === 0x01 && type === 0x75) {
      ...
      // PIR: 1 byte trigger/idle (datasheet + TTN example form)
      ...
      // PIR: uint16 LE activity event count (AM107-style form)
