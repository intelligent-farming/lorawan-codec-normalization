...
//
// Normalized payload codec for Elsys ERS VOC (indoor air-quality multi-sensor:
// temperature, humidity, light, motion/PIR, CO2, and TVOC).
//
...
// with action.motion.detected = count > 0. TVOC (type 0x1c, ppb) has no
// vocabulary key, so it is emitted as the camelCase extra `tvoc`. Fields with no
// vocabulary key (acceleration, analog input, external temperature, sound,
// distance, etc.) are emitted as camelCase extras.
...
function decodeUplinkCore(input) {
  ...
  while (i < bytes.length) {
    ...
    if (type === 0x01) {
      ...
      // TVOC: 2 bytes unsigned, ppb. No vocab key -> camelCase extra.
