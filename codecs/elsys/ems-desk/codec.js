...
//
// Normalized payload codec for Elsys EMS Desk (desk-mounted occupancy sensor
// with temperature, humidity, accelerometer, and PIR occupancy).
//
...
// (type 0x05) and occupancy (type 0x11) are both counts of detected presence
// events, mapped to action.motion.count with action.motion.detected = count > 0.
// Fields with no vocabulary key (acceleration axes) are emitted as camelCase
// extras.
...
function decodeUplinkCore(input) {
  ...
  while (i < bytes.length) {
    ...
    if (type === 0x01) {
      ...
      var mcount = bytes[i + 1];
      motion.detected = mcount > 0;
      motion.count = mcount;
      ...
      // Occupancy: 1 byte, presence count. Mapped to action.motion.
      var ocount = bytes[i + 1];
      motion.detected = ocount > 0;
      motion.count = ocount;
      action.motion = motion;
      i += 2;
      recognized = true;
