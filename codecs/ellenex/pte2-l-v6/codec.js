...
//
// Normalized payload codec for ellenex/pte2-l-v6 (PTE2-L - Earth Pressure Transmitter).
//
...
//
// Keys: v -> battery (mV); P -> pressure.gauge (kg/cm2 -> kPa, x98.0665)
function decodeUplinkCore(input) {
  ...
  for (k in parsed) {
    ...
    else if (k === 'P') { if (typeof value === 'number') { data.pressure = { gauge: round(value * 98.0665, 3) }; produced = true; } }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "ellenex"; result.data.model = "pte2-l-v6"; }
  return result;
}
