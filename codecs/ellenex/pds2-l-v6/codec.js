...
//
// Normalized payload codec for ellenex/pds2-l-v6 (PDS2-L - Differential Pressure Transmitter).
//
...
//
// Keys: v -> battery (mV); DP -> pressure.differential (bar -> Pa, x100000)
function decodeUplinkCore(input) {
  ...
  for (k in parsed) {
    ...
    else if (k === 'DP') { if (typeof value === 'number') { data.pressure = { differential: round(value * 100000, 3) }; produced = true; } }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "ellenex"; result.data.model = "pds2-l-v6"; }
  return result;
}
