...
//
// Normalized payload codec for ellenex/ctr2-l-v6 (CTR2-L - Turbidity Sensor).
//
...
// (github.com/ellenex/lorawan-payload-decoders, Version 6 Sensors/CTR2-L),
...
//
// Keys: v -> battery (mV); T -> water.temperature.current (C); TR1 -> water.turbidity (NTU); TR2 (FNU)/TR3 (mg/l) -> extras
function decodeUplinkCore(input) {
  ...
  for (k in parsed) {
    ...
    else if (k === 'TR1') { data.water = data.water || {}; data.water.turbidity = round(value, 2); produced = true; }
    else if (k === 'TR2') { data.turbidityFnu = round(value, 2); }
    else if (k === 'TR3') { data.turbidityMgl = round(value, 2); }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "ellenex"; result.data.model = "ctr2-l-v6"; }
  return result;
}
