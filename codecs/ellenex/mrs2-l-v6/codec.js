...
//
// Normalized payload codec for ellenex/mrs2-l-v6 (MRS2-L - Tipping Spoon Rain Sensor).
//
...
//
// Keys: v -> battery (mV); Pu -> rain.cumulative (pulses x 0.2 mm/tip)
function decodeUplinkCore(input) {
  ...
  for (k in parsed) {
    ...
    else if (k === 'Pu') { if (typeof value === 'number') { data.rain = { cumulative: round(value * 0.2, 3) }; data.rainTipCount = value; produced = true; } }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "ellenex"; result.data.model = "mrs2-l-v6"; }
  return result;
}
