...
//
// Normalized payload codec for ellenex/ptd2-l-v6 (PTD2-L - Pressure Transmitter with Temperature).
//
...
//
// Keys: v -> battery (mV); P -> pressure.gauge (bar -> kPa, x100); T -> air.temperature (C)
function decodeUplinkCore(input) {
  ...
  for (k in parsed) {
    ...
    else if (k === 'P') { if (typeof value === 'number') { data.pressure = { gauge: round(value * 100, 3) }; produced = true; } }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "ellenex"; result.data.model = "ptd2-l-v6"; }
  return result;
}
