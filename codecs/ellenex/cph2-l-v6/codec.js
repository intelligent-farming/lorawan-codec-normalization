...
//
// Normalized payload codec for ellenex/cph2-l-v6 (CPH2-L - pH Sensor).
//
...
// (github.com/ellenex/lorawan-payload-decoders, Version 6 Sensors/CPH2-L),
...
//
// Keys: v -> battery (mV); T -> water.temperature.current (C); pH1 -> water.ph; Red -> water.orp (mV); pH2 -> ph2Mv extra
function decodeUplinkCore(input) {
  ...
  for (k in parsed) {
    ...
    else if (k === 'pH1') { data.water = data.water || {}; data.water.ph = round(value, 2); produced = true; }
    else if (k === 'Red') { data.water = data.water || {}; data.water.orp = round(value, 1); produced = true; }
    else if (k === 'pH2') { data.ph2Mv = round(value, 1); }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "ellenex"; result.data.model = "cph2-l-v6"; }
  return result;
}
