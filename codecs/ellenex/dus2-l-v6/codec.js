...
//
// Normalized payload codec for ellenex/dus2-l-v6 (DUS2-L-V6 - Ultrasonic Distance / Level Sensor).
//
// Ellenex "Version 6" firmware emits a CBOR-encoded map (fPort 15) keyed by
// short sensor codes carrying calibrated SI values. Original work; CBOR key set
// and units from Ellenex's published V6 decoder (github.com/ellenex/
// lorawan-payload-decoders), reproduced as facts. CBOR reader shared with
// ellenex/pls2-l-v6 (authored from the Apache-2.0 TTN reference).
//
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "ellenex"; result.data.model = "dus2-l-v6"; }
  return result;
}
