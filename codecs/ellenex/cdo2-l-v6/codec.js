...
//
// Normalized payload codec for ellenex/cdo2-l-v6 (CDO2-L - Dissolved Oxygen Sensor).
//
...
// (github.com/ellenex/lorawan-payload-decoders, Version 6 Sensors/CDO2-L),
// reproduced as facts — no upstream decoder copied. The CBOR reader is shared
// with ellenex/pls2-l-v6 (authored from the Apache-2.0 TTN reference).
//
// Keys: v -> battery (mV); T -> water.temperature.current (C); DO2 -> water.dissolvedOxygen (mg/L); DO1 (%Sat)/DO3 (ppm) -> extras
function decodeUplinkCore(input) {
  ...
  for (k in parsed) {
    ...
    else if (k === 'T') { if (typeof value === 'number') { data.water = data.water || {}; data.water.temperature = { current: round(value, 2) }; } }
    else if (k === 'DO2') { data.water = data.water || {}; data.water.dissolvedOxygen = round(value, 2); produced = true; }
    else if (k === 'DO1') { data.dissolvedOxygenPercentSat = round(value, 2); }
    else if (k === 'DO3') { data.dissolvedOxygenPpm = round(value, 2); }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "ellenex"; result.data.model = "cdo2-l-v6"; }
  return result;
}
