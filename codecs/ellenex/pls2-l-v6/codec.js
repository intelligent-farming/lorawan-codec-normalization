...
//
// Normalized payload codec for ellenex/pls2-l-v6 (Ellenex PLS2-L-V6 LoRaWAN
// low-power liquid-level / hydrostatic-level transmitter).
//
// Ported from the upstream Apache-2.0 reference decoder
// (TheThingsNetwork/lorawan-devices vendor/ellenex/pls2-l-v6.js, codecId
// "pls2-l-v6-codec", attributed in NOTICE). The wire format (a CBOR-encoded
// map keyed by short sensor codes) is faithful to upstream; normalization to
// the shared vocabulary is authored here (we do NOT reuse upstream's output
// shape or its SENSOR_MAP names).
//
// Wire format: the payload is a CBOR object (typically an indefinite-length
// map, major type 5, head 0xBF ... break 0xFF) whose keys are 1-character
// text strings:
//   "L" -> liquid-column level above the sensor, IEEE-754 float (metres)
//   "v" -> battery voltage, unsigned integer (millivolts)
// The CBOR reader below supports the subset upstream emits: unsigned/negative
// integers, byte/text strings, arrays, maps (definite and indefinite),
// simple values, and half/single/double floats. BigInt-free: 64-bit lengths
// are folded with Math (sufficient for these short payloads).
//
// Canonical mapping:
//   L (level, m)        -> water.level (m)        [satisfies groundwater]
//   v (battery, mV)     -> battery (V, mV / 1000)
//   any other CBOR key  -> camelCase extra (raw value, unconverted)
...
function decodeUplinkCore(input) {
  ...
  try {
    ...
    return { errors: ['CBOR decode failed: ' + e.message] };
  }

  if (parsed === null || typeof parsed !== 'object' || isArray(parsed)) {
    return { errors: ['payload is not a CBOR map'] };
  }
  ...
  for (k in parsed) {
    ...
    if (k === 'L') {
      if (typeof value !== 'number' || !isFinite(value)) {
        return { errors: ['invalid level value'] };
      }
      ...
      if (typeof value !== 'number' || !isFinite(value)) {
        return { errors: ['invalid battery value'] };
      }
      data.battery = round(value / 1000, 3);
      produced = true;
    } else {
      // Genuine non-vocabulary field: keep as a camelCase extra, raw.
      data[camel(k)] = value;
    }
  }
...
function decodeUplink(input) {
  ...
  if (result && result.data) {
    result.data.make = "ellenex";
    result.data.model = "pls2-l-v6";
  }
  return result;
}
