...
//
// Normalized payload codec for ellenex/duc2-l-v6 (DUC2-L-V6 - Ultrasonic Distance / Level Sensor (corrosion-resistant)).
//
// Ellenex "Version 6" firmware emits a CBOR map (fPort 15) of calibrated SI
// values. Original work; CBOR key set/units from Ellenex's published V6 decoder
// (github.com/ellenex/lorawan-payload-decoders), reproduced as facts. CBOR reader
// shared with ellenex/pls2-l-v6 (authored from the Apache-2.0 TTN reference).
//
// Keys: v -> battery (mV); D -> tank.distance (m, distance to surface)
function decodeUplinkCore(input) {
  ...
  for (k in parsed) {
    ...
    else if (k === 'D') { if (typeof value === 'number') { data.tank = { distance: round(value, 3) }; produced = true; } }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "ellenex"; result.data.model = "duc2-l-v6"; }
  return result;
}
