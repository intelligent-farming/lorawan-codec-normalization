...
//
// Normalized payload codec for ellenex/tts3-l-v6 (TTS3-L-V6 - Temperature Transmitter (IP68)).
//
...
//
// Keys: v -> battery (mV); T -> temperature (C)
function decodeUplinkCore(input) {
  ...
  for (k in parsed) {
    ...
    else if (k === 'T') { if (typeof value === 'number') { data.temperature = round(value, 2); produced = true; } }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "ellenex"; result.data.model = "tts3-l-v6"; }
  return result;
}
