...
//
// Normalized payload codec for ellenex/mas2-l-v6 (MAS2-L - Humidity & Temperature Sensor).
//
...
// (github.com/ellenex/lorawan-payload-decoders, Version 6 Sensors/MAS2-L),
...
//
// Keys: v -> battery (mV); RH -> air.relativeHumidity (%); T -> air.temperature (C)
function decodeUplinkCore(input) {
  ...
  for (k in parsed) {
    ...
    else if (k === 'RH') { data.air = data.air || {}; data.air.relativeHumidity = round(value, 1); produced = true; }
...
function decodeUplink(input) {
  ...
  if (result && result.data) { result.data.make = "ellenex"; result.data.model = "mas2-l-v6"; }
  return result;
}
